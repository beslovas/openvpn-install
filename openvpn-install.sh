...
# https://github.com/beslovas/openvpn-install
#
# Copyright (c) 2013 Nyr. Released under the MIT License.

if [ $1 == "--autoscript" ]; then
	autoscript=true
	protocol=udp
	port=1194
	dns=1
...
if [[ ! -e /etc/openvpn/server/server.conf ]]; then
	...
	if echo "$ip" | grep -qE '^(10\.|172\.1[6789]\.|172\.2[0-9]\.|172\.3[01]\.|192\.168)' || [[ -n "$NAT" ]]; then
		echo
		...
		[[ -z "$public_ip" ]] && public_ip="$get_public_ip" && echo "Using $get_public_ip as public IPv4 address."
	fi
...
LimitNPROC=infinity" > /etc/systemd/system/openvpn-server@server.service.d/disable-limitnproc.conf
	...
	if [[ "$os" = "debian" || "$os" = "ubuntu" ]]; then
		...
		apt-get install -y openssl ca-certificates $firewall
	elif [[ "$os" = "centos" ]]; then
